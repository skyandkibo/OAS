...
class OBBoxHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.loss_angle_flag = loss_angle_flag
        if self.loss_angle_flag:
        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                ...
                if self.loss_angle_flag:
                    ...
                    losses_angle = self.loss_angle(
                        pos_bbox_pred[:, -1],
                        bbox_targets[pos_inds.type(torch.bool)][:, -1],
                        bbox_weights[pos_inds.type(torch.bool)][:, -1] * w_i_all,
                        avg_factor=bbox_targets.size(0),
                        reduction_override=reduction_override)
                else:
                    losses['loss_bbox'] = self.loss_bbox(
                ...
                if self.loss_angle_flag:
                    losses['loss_bbox'] = bbox_pred[:,:-1].sum() * 0
                    losses_angle = bbox_pred[:,-1].sum() * 0
                else:
                    losses['loss_bbox'] = bbox_pred.sum() * 0

        if self.loss_angle_flag:
            return losses, losses_angle
        else:
            return losses, None
