...
class OBBStandardRoIHead(OBBBaseRoIHead, OBBoxTestMixin):
    ...
    def init_assigner_sampler(self):
        ...
        self.berta = nn.Parameter(torch.tensor(2.0))
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            ...
            for i in range(num_imgs):
                ...
                w_i = self.compute_w_i(sampling_result)
                sampling_results.append(sampling_result)
                w_i_results.append(w_i)

        losses = dict()
        # bbox head forward and loss
        if self.with_bbox:
            w_i_all = torch.cat(w_i_results, dim=0)  
            bbox_results, loss_angle = self._bbox_forward_train(x, sampling_results, w_i_all,
                                                    gt_bboxes, gt_labels,
                                                    img_metas)
    ...
    def _bbox_forward_train(self, x, sampling_results, w_i_all, gt_bboxes, gt_labels,
                            img_metas):
        ...
        loss_bbox, loss_angle = self.bbox_head.loss(bbox_results['cls_score'],
                                        bbox_results['bbox_pred'], w_i_all, rois,
                                        *bbox_targets)

        bbox_results.update(loss_bbox=loss_bbox)
        return bbox_results, loss_angle
    ...
    def compute_w_i(self,sampling_result):
        diff = torch.abs(sampling_result.pos_bboxes[:,-1] - sampling_result.pos_gt_bboxes[:,-1])
        now_device=diff.device

        pos_l = torch.where(diff > torch.pi / 2)
            
        angle_pos_l_result = diff[pos_l]

        pos_l_new = torch.flip(angle_pos_l_result.argsort(), dims=[0])
        s_i = torch.ones(diff.numel()).to(now_device)

        exponent = -self.berta * ((pos_l[0].numel() - torch.arange(pos_l[0].numel()).to(now_device)) / (pos_l[0].numel() + 1))

        s_i[pos_l[0][pos_l_new]] = torch.exp(exponent)

        e_sum = torch.sum(torch.exp(s_i))

        w_i = diff.numel() * torch.exp(s_i) / e_sum
